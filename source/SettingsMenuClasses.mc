...
class GeneralMenu extends WatchUi.Menu2 {
  function initialize() {
    ...
    memoryCache = null;
    Menu2.initialize({
      :title => Application.loadResource(Rez.Strings.SettingsMenu),
    });
    addItem(
      new TogleItem(
        :SwitchDayNight,
        "SwitchDayNight",
        Rez.Strings.SwitchDayNight
      )
    );
    ...
    addItem(
      new Item(
        ...
        Rez.Strings.T1TZ,
        null,
        Application.Properties.getValue("T1TZ").toString()
      )
    );
    addItem(
      new Item(
    ...
    addItem(
      new Item(
        ...
        Rez.Strings.PrU,
        :pressureUnit,
        SettingsReference.getSublabelForField(
          SettingsReference.pressureUnit(),
          "PrU"
        )
      )
    );
    addItem(
      new Item(
        ...
        :windSpeedUnit,
        SettingsReference.getSublabelForField(
          SettingsReference.windSpeedUnit(),
          "WU"
        )
      )
    );
  ...
  function initialize(mode) {
    ...
    addItem(
      new Item(
        :WTypeTop,
        dictNames[:WTypeTop],
        Rez.Strings.WTypeTop,
        :widgetTypeTop,
        SettingsReference.getSublabelForField(
          SettingsReference.widgetTypeTop(),
          dictNames[:WTypeTop]
        )
      )
    );
    addItem(
      new Item(
        :WTypeBottom,
        dictNames[:WTypeBottom],
        Rez.Strings.WTypeBottom,
        :widgetTypeBottom,
        SettingsReference.getSublabelForField(
          SettingsReference.widgetTypeBottom(),
          dictNames[:WTypeBottom]
        )
      )
    );
    ...
    addItem(
      new Item(
        ...
        Rez.Strings.DF,
        null,
        Application.Properties.getValue(dictNames[:DF])
      )
    );
  }
}
...
class MenuFieldsList extends WatchUi.Menu2 {
  function initialize(listId, mode, title) {
    ...
    if (listId == :FS) {
      ...
      addItem(
        createItem(:SF0, Rez.Strings.SF0, :statusField, dictNames, dictValues)
      );
      addItem(
        createItem(:SF1, Rez.Strings.SF1, :statusField, dictNames, dictValues)
      );
      addItem(
        createItem(:SF2, Rez.Strings.SF2, :statusField, dictNames, dictValues)
      );
      addItem(
        createItem(:SF3, Rez.Strings.SF3, :statusField, dictNames, dictValues)
      );
      addItem(
        createItem(:SF4, Rez.Strings.SF4, :statusField, dictNames, dictValues)
      );
      addItem(
        createItem(:SF5, Rez.Strings.SF5, :statusField, dictNames, dictValues)
      );
    }
  }
  ...
  function onSelect(mode) {
    if (subMenuDictonarySymbol != null) {
      var subMenu = new SelectMenu(
        getLabel(),
        (
          new Toybox.Lang.Method(SettingsReference, subMenuDictonarySymbol)
        ).invoke(),
        propName
      );
      WatchUi.pushView(
        subMenu,
        new SelectMenuDelegate(self.weak(), propName),
      ...
      if (itemId == :G || itemId == :D || itemId == :N) {
        ...
        WatchUi.pushView(
          subMenu,
          new GeneralMenuDelegate(itemId),
          WatchUi.SLIDE_IMMEDIATE
        );
      } else {
      ...
      if (WatchUi has :Picker) {
        var picker = new StringPicker(
          self,
          SettingsReference.getCharsString(getId())
        );
        WatchUi.pushView(
          picker,
...
class SelectItem extends WatchUi.MenuItem {
  ...
  function initialize(identifier, value) {
    self.value = value;
    MenuItem.initialize(
      Application.loadResource(Rez.Strings[identifier]),
      null,
      identifier,
      {}
    );
