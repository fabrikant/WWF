...
class WWFView extends WatchUi.WatchFace {
  ...
  function initialize() {
    ...
    Application.Properties.setValue(
      "keyOW",
      "69bcb8de48220cd2b2fb7a8400c68d1e"
    );
  }
  ...
  function createFields(dc) {
    ...
    if (wType == EMPTY) {
      ...
      wType == WIDGET_TYPE_WEATHER ||
      wType == WIDGET_TYPE_WEATHER_WIND ||
      wType == WIDGET_TYPE_WEATHER_FIELDS
    ) {
      topBarWidth += createWeatherFields(currentTop, h, w, [
        :weather_picture,
        :weather_temp,
      ]);
    ...
    fieldsBarWidth += createDataField(
      :P2,
      :F2,
      fieldsBarWidth,
      y,
