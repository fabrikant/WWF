...
module Tools {
  ...
  function minutesToString(rawData) {
    ...
    return Toybox.Lang.format("$1$:$2$", [
      hour.format("%02d"),
      (rawData - hour * 60).format("%02d"),
    ]);
  }
  ...
  function getSunEvent(event, allowTomorrow) {
    ...
    var today = Time.today().add(
      new Time.Duration(12 * Gregorian.SECONDS_PER_HOUR)
    );
    ...
    if (memoryCache.sunEvents[tomorrow] == null) {
      ...
      memoryCache.sunEvents = {
        today => {
          SUNRISE => sunEventCalculator.calculate(
            today,
            myLocation[0],
            myLocation[1],
            SUNRISE
          ),
          SUNSET => sunEventCalculator.calculate(
            today,
            myLocation[0],
            myLocation[1],
            SUNSET
  ...
  function weekOfYear(moment) {
    ...
    var jan1DayOfWeek = Gregorian.info(
      jan1,
    ...
    if (jan1DayOfWeek < 5) {
      ...
      return (
        1 +
        beginMoment.subtract(moment).value() / (Gregorian.SECONDS_PER_DAY * 7)
      );
    } else {
      if (
        momentInfo.month == 1 &&
        momentInfo.day <= 3 &&
        (momentInfo.day_of_week == 1 || momentInfo.day_of_week > 5)
      ) {
        return weekOfYear(
          Gregorian.moment({
            :year => momentInfo.year - 1,
        ...
        var beginMoment = jan1.add(
          new Time.Duration((8 - jan1DayOfWeek) * Gregorian.SECONDS_PER_DAY)
        );
        return (
          1 +
          beginMoment.subtract(moment).value() / (Gregorian.SECONDS_PER_DAY * 7)
